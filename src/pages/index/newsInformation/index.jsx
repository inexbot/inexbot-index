              ...
              {DataList === null? "": DataList[newsSelect].slice(0,6).map((item,index)=>{
                return (
                  <div key={index} className={style.NewsInformation_content_r} style={index ===0?{display:"none"}:index === 1?{}:{marginTop:"10px"}} onClick={()=>{
                    ...
                    <div>
                      <p> {item.title} </p>
                      <span className={style.NewsInformation_content_txt1}> { item.id===241? item.description.slice(0,40):item.description.slice(0,20)} </span>
                      <span className={style.NewsInformation_content_txt2} style={ props.BannerWidth > 1200? item.id===241?item.description.length>40?{display:"block"} :{display:"none"}:item.description.length<20?{display:"none"}:{display:"block"}:{display:"none"}}>
                        {item.id===241? item.description.slice(40):item.description.slice(20)}
                      </span>
                      <span className={style.NewsInformation_content_date} >{`
